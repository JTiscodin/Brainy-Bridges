...
//         echo "<button onclick=\"startQuiz('2', '$subject_name', 'endterm')\">Start Quiz</button>";
//         echo "</div>";
//     }
// } else {
//     echo "0 results";
...
<body>
    ...
    <script>
        function startQuiz(year, subject, quizType) {
            // Redirect to the corresponding quiz page
            var key = year + "_" + subject + "_" + quizType;
            var quizPages = {
                // Quiz pages will be dynamically generated
            };
            if (quizPages.hasOwnProperty(key)) {
                window.location.href = quizPages[key];
            } else {
                alert("No quiz available for the selected options.");
            }
        }
    </script>
</body>
